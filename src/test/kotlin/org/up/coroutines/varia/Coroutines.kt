...
fun getRating(p: Product): CompletableFuture<Int> =
    CompletableFuture.supplyAsync {
        if (p.id > 1000) throw IllegalArgumentException("not found") else 10
    }
...
fun memoryUsageCoroutines() =
    runBlocking {
        val cr =
            (1..2_400_000).toList().map {
                async {
...
suspend fun <T> CompletableFuture<T>.await2(): T =
...
suspend fun receive(channel: Channel<String>) =
    coroutineScope {
        val received = channel.receive()
