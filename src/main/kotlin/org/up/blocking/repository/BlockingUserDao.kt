...
@Component
class BlockingEnrollmentService(
    @Value("\${remote.service.delay.ms}")val delayCfg: Long,
    @Value("\${remote.service.url}")val baseUrl: String,
) {
    val restTemplate = RestTemplate()
    ...
    fun verifyEmail(
        ...
        restTemplate.getForEntity("$baseUrl/echo?email=$email&value=true&delay=${delay ?: delayCfg}", String::class.java).body!! ==
            "true".also {
